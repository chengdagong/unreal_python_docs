# unreal.MovieSceneScriptingTextChannel

```python
class unreal.MovieSceneScriptingTextChannel(outer:Object | None=None, name:Name | str='None')
```
...
Movie Scene Scripting Text Channel
...
**add_key**( _time_, _new_value_, _sub_frame=0.000000_, _time_unit=MovieSceneTimeUnit.DISPLAY_RATE_) → `MovieSceneScriptingTextKey`
...
- **new\_value** ( _Text_) – The value that this key should be created with.
...
Returns:\
\
An array of UMovieSceneScriptingTextKey’s contained by this channel. Returns all keys even if clipped by the owning section’s boundaries or outside of the current sequence play range.\
\
Return type:\
\
...
\
- `unreal.MovieSceneScriptingTextChannel`\
  - `MovieSceneScriptingTextChannel`\
    - `MovieSceneScriptingTextChannel.add_key()`\
    - `MovieSceneScriptingTextChannel.get_default()`\
    - `MovieSceneScriptingTextChannel.get_keys()`\
    - `MovieSceneScriptingTextChannel.get_keys_by_index()`\
    - `MovieSceneScriptingTextChannel.has_default()`\
    - `MovieSceneScriptingTextChannel.remove_default()`\
    - `MovieSceneScriptingTextChannel.remove_key()`\
    - `MovieSceneScriptingTextChannel.set_default()`\
    - `MovieSceneScriptingTextChannel.transform()`\
\