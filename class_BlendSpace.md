...
```python
class unreal.BlendSpace(outer:Object | None=None, name:Name | str='None')
```
...
- `should_match_sync_phases` (bool): [Read-Write] If true, all follower samples will pass the same amount of markers the leader sample has passed to match its sync phase. Otherwise, followers samples will only match their next valid sync position.
