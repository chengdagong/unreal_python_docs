...
```python
class unreal.MeshComponent(outer:Object | None=None, name:Name | str='None')
```
...
_property_ `enable_material_parameter_caching`: bool

[Read-Only] Whether or not to cache material parameter to speed up setting scalar or vector value on materials
...
**get_materials**() → `Array\ [MaterialInterface]`
...
_property_ `material_slots_overlay_material`: None

[Read-Only] Translucent material to blend on top of this mesh. Mesh will be rendered twice - once with a base material and once with overlay material.
The difference with the global OverlayMaterial is those are per material slot, if the entry is null or doesn’t exist the global
OverlayMaterial will be use for sections using the material slot.
...
[Read-Only] The max draw distance for overlay material. A distance of 0 indicates that overlay will be culled using primitive max distance.
...
**prestream_mesh_lo_ds**( _seconds:float_) → `bool`

deprecated: ‘prestream\_mesh\_lo\_ds’ was renamed to ‘prestream\_mesh\_lods’.


---

**prestream_mesh_lods**( _seconds_) → `boolTell the streaming system to start streaming in all LODs for the mesh.`

Note: this function may set bIgnoreStreamingMipBias on this component enable the FastForceResident system.

Parameters:

**seconds** ( _float_) – Number of seconds to force all LODs to be resident
...
**prestream_textures**( _seconds_, _prioritize_character_textures_, _cinematic_texture_groups=0_) → `None`
...
- **prioritize\_character\_textures** ( _bool_) – Whether character textures should be prioritized for a while by the streaming system
...
**set_color_parameter_value_on_materials**( _parameter_name_, _parameter_value_) → `None`
...
- **parameter\_value** ( _LinearColor_)
...
**set_scalar_parameter_value_on_materials**( _parameter_name_, _parameter_value_) → `None`
...
- **parameter\_value** ( _float_)



---

**set_vector_parameter_value_on_materials**( _parameter_name_, _parameter_value_) → `None`
...
- `unreal.MeshComponent`
  - `MeshComponent`
    - `MeshComponent.enable_material_parameter_caching`
    - `MeshComponent.get_materials()`
    - `MeshComponent.get_overlay_material()`
    - `MeshComponent.get_overlay_material_max_draw_distance()`
    - `MeshComponent.material_slots_overlay_material`
    - `MeshComponent.overlay_material`
    - `MeshComponent.overlay_material_max_draw_distance`
    - `MeshComponent.prestream_mesh_lo_ds()`
    - `MeshComponent.prestream_mesh_lods()`
    - `MeshComponent.prestream_textures()`
    - `MeshComponent.set_color_parameter_value_on_materials()`
    - `MeshComponent.set_overlay_material()`
    - `MeshComponent.set_overlay_material_max_draw_distance()`
    - `MeshComponent.set_scalar_parameter_value_on_materials()`
    - `MeshComponent.set_vector_parameter_value_on_materials()`