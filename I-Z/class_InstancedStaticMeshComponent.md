...
```python
class unreal.InstancedStaticMeshComponent(outer:Object | None=None, name:Name | str='None')
```


**Bases:** ``StaticMeshComponent``
...
- `instance_min_draw_distance` (int32): [Read-Write] Distance from camera at which each instance begins to draw.
...
**acquire_editor_element_handle**( _instance_index_, _allow_create=True_) → `ScriptTypedElementHandle`

K2 Acquire Editor SMInstance Element Handle

Parameters:

- **instance\_index** ( _int32_)
...
**add_instance_world_space**( _world_transform_) → `int32`
...
**add_instances**( _instance_transforms_, _should_return_indices_, _world_space=False_, _update_navigation=True_) → `Array[int32]`

Add multiple instances to this component. Transform is given in local space of this component unless bWorldSpace is set.

Parameters:

- **instance\_transforms** ( _Array_ _\_ [_Transform_ _]_)

- **should\_return\_indices** ( _bool_)

- **world\_space** ( _bool_)

- **update\_navigation** ( _bool_)
...
**batch_update_instances_transform**( _start_instance_index_, _num_instances_, _new_instances_transform_, _world_space=False_, _mark_render_state_dirty=False_, _teleport=False_) → `bool`

Update the transform for a number of instances.

Parameters:

- **start\_instance\_index** ( _int32_) – The starting index of the instances to update

- **num\_instances** ( _int32_) – The number of instances to update

- **new\_instances\_transform** ( _Transform_) – The new transform

- **world\_space** ( _bool_) – If true, the new transform is interpreted as a World Space transform, otherwise it is interpreted as Local Space

- **mark\_render\_state\_dirty** ( _bool_) – If true, the change should be visible immediately. If you are updating many instances you should only set this to true for the last instance.

- **teleport** ( _bool_) – Whether or not the instances physics should be moved normally, or teleported (moved instantly, ignoring velocity).
...
**batch_update_instances_transforms**( _start_instance_index_, _new_instances_transforms_, _world_space=False_, _mark_render_state_dirty=False_, _teleport=False_) → `bool`
...
- **new\_instances\_transforms** ( _Array_ _\_ [_Transform_ _]_) – The new transforms

- **world\_space** ( _bool_) – If true, the new transforms are interpreted as a World Space transform, otherwise it is interpreted as Local Space
...
get\_cull\_distances( _)->(out\_start\_cull\_distance=int32_, _out\_end\_cull\_distance=int32_)

Gets the fading start and culling end distances for this component.

Returns:

out\_start\_cull\_distance (int32):

out\_end\_cull\_distance (int32):
...
**get_instance_transform**( _instance_index_, _world_space=False_) → `TransformorNone`
...
out\_instance\_transform (Transform):
...
**get_instances_overlapping_box**( _box_, _box_in_world_space=True_) → `Array[int32]`
...
**get_instances_overlapping_sphere**( _center_, _radius_, _sphere_in_world_space=True_) → `Array[int32]`
...
- **sphere\_in\_world\_space** ( _bool_)
...
_property_ `instance_end_cull_distance`: int

[Read-Only] Distance from camera at which each instance completely fades out.
...
_property_ `instance_lod_distance_scale`: float

[Read-Only] Scale applied to change the computation of LOD distances when using the StaticMesh screen sizes.
Smaller values make LODs transition earlier.
...
_property_ `instance_min_draw_distance`: int

[Read-Only] Distance from camera at which each instance begins to draw.
...
_property_ `instance_start_cull_distance`: int

[Read-Only] Distance from camera at which each instance begins to fade out.
...
_property_ `instancing_random_seed`: int

[Read-Write] Value used to seed the random number stream that generates random numbers for each of this mesh’s instances.
The random number is stored in a buffer accessible to materials through the PerInstanceRandom expression. If
this is set to zero (default), it will be populated automatically by the editor.
...
**is_valid_instance**( _instance_index_) → `bool`
...
**remove_instance**( _instance_index_) → `bool`
...
**remove_instances**( _instances_to_remove_) → `bool`

Remove the instances specified. Returns True on success.

Parameters:

**instances\_to\_remove** ( _Array_ _[_ _int32_ _]_)
...
**set_cull_distances**( _start_cull_distance_, _end_cull_distance_) → `None`

Sets the fading start and culling end distances for this component.

Parameters:

- **start\_cull\_distance** ( _int32_)

- **end\_cull\_distance** ( _int32_)



---

**set_custom_data_value**( _instance_index_, _custom_data_index_, _custom_data_value_, _mark_render_state_dirty=False_) → `bool`
...
- **custom\_data\_index** ( _int32_)

- **custom\_data\_value** ( _float_)

- **mark\_render\_state\_dirty** ( _bool_)
...
**set_lod_distance_scale**( _lod_distance_scale_) → `None`

Sets the LOD scale.

Parameters:

**lod\_distance\_scale** ( _float_)


---

**set_num_custom_data_floats**( _num_custom_data_floats_) → `None`
...
**num\_custom\_data\_floats** ( _int32_)


---

**update_instance_transform**( _instance_index_, _new_instance_transform_, _world_space=False_, _mark_render_state_dirty=False_, _teleport=False_) → `bool`

Update the transform for the instance specified.

Parameters:

- **instance\_index** ( _int32_) – The index of the instance to update

- **new\_instance\_transform** ( _Transform_) – The new transform

- **world\_space** ( _bool_) – If true, the new transform is interpreted as a World Space transform, otherwise it is interpreted as Local Space

- **mark\_render\_state\_dirty** ( _bool_) – If true, the change should be visible immediately. If you are updating many instances you should only set this to true for the last instance.

- **teleport** ( _bool_) – Whether or not the instance’s physics should be moved normally, or teleported (moved instantly, ignoring velocity).
...
_property_ `use_gpu_lod_selection`: bool

[Read-Only] If true, this component will use GPU LOD selection.
...
- `unreal.InstancedStaticMeshComponent`
  - `InstancedStaticMeshComponent`
    - `InstancedStaticMeshComponent.acquire_editor_element_handle()`
    - `InstancedStaticMeshComponent.add_instance()`
    - `InstancedStaticMeshComponent.add_instance_world_space()`
    - `InstancedStaticMeshComponent.add_instances()`
    - `InstancedStaticMeshComponent.batch_update_instances_transform()`
    - `InstancedStaticMeshComponent.batch_update_instances_transforms()`
    - `InstancedStaticMeshComponent.clear_instances()`
    - `InstancedStaticMeshComponent.get_cull_distances()`
    - `InstancedStaticMeshComponent.get_instance_count()`
    - `InstancedStaticMeshComponent.get_instance_transform()`
    - `InstancedStaticMeshComponent.get_instances_overlapping_box()`
    - `InstancedStaticMeshComponent.get_instances_overlapping_sphere()`
    - `InstancedStaticMeshComponent.get_lod_distance_scale()`
    - `InstancedStaticMeshComponent.instance_end_cull_distance`
    - `InstancedStaticMeshComponent.instance_lod_distance_scale`
    - `InstancedStaticMeshComponent.instance_min_draw_distance`
    - `InstancedStaticMeshComponent.instance_start_cull_distance`
    - `InstancedStaticMeshComponent.instancing_random_seed`
    - `InstancedStaticMeshComponent.is_valid_instance()`
    - `InstancedStaticMeshComponent.remove_instance()`
    - `InstancedStaticMeshComponent.remove_instances()`
    - `InstancedStaticMeshComponent.set_cull_distances()`
    - `InstancedStaticMeshComponent.set_custom_data_value()`
    - `InstancedStaticMeshComponent.set_lod_distance_scale()`
    - `InstancedStaticMeshComponent.set_num_custom_data_floats()`
    - `InstancedStaticMeshComponent.update_instance_transform()`
    - `InstancedStaticMeshComponent.use_gpu_lod_selection`