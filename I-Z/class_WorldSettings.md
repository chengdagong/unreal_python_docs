...
```python
class unreal.WorldSettings(outer:Object | None=None, name:Name | str='None')
```
...
- `asset_user_data_editor_only` (Array[AssetUserData]): [Read-Write] Array of editor-only user data stored with the asset
...
- `default_bookmark` (BookmarkBase): [Read-Write] Bookmark that will be applied when the level is loaded in the editor.
...
deprecated: As of UE 5.1 all worlds are large. Set UE\_USE\_UE4\_WORLD\_MAX in EngineDefines.h to alter this.
...
- `global_gravity_z` (float): [Read-Write] Set a custom gravity value for this level, in distance units per second squared.
...
- `max_cinematic_time_dilation` (float): [Read-Write] Highest acceptable cinematic time dilation.
...
- `min_cinematic_time_dilation` (float): [Read-Write] Lowest acceptable cinematic time dilation.
...
_property_ `ai_system_class`: Class
...
_property_ `default_color_scale`: Vector

[Read-Only] Default color scale for the level
...
_property_ `default_game_mode`: Class
...
_property_ `default_game_type`: Class

‘default\_game\_type’ was renamed to ‘default\_game\_mode’.
...
_property_ `default_physics_volume_class`: Class
...
_property_ `enable_ai_system`: bool
...
_property_ `enable_large_worlds`: bool
...
_property_ `enable_navigation_system`: bool
...
_property_ `enable_world_bounds_checks`: bool
...
_property_ `enable_world_composition`: bool

[Read-Only] Enables tools for composing a tiled world.
Level has to be saved and all sub-levels removed before enabling this option.
...
_property_ `enable_world_origin_rebasing`: bool
...
_property_ `global_gravity_set`: bool
...
_property_ `global_gravity_z`: float

[Read-Only] Set a custom gravity value for this level, in distance units per second squared.
For example, when the project’s distance unit is set to centimeters (the default), normal earth gravity would be -980.0.
...
_property_ `kill_z`: float

[Read-Only] any actor falling below this level gets destroyed
...
_property_ `kill_z_damage_type`: Class
...
_property_ `lightmass_settings`: LightmassWorldInfoSettings

[Read-Write] LIGHTMASS RELATED SETTINGS \*
...
_property_ `navigation_system_config`: NavigationSystemConfig

[Read-Only] Holds parameters for NavigationSystem’s creation. Set to Null will result
in NavigationSystem instance not being created for this world. Note that
if set NavigationSystemConfigOverride will be used instead.
...
_property_ `physics_collision_handler_class`: Class
...
_property_ `use_client_side_level_streaming_volumes`: bool

[Read-Only] Enables client-side streaming volumes instead of server-side.
Expected usage scenario: server has all streaming levels always loaded, clients independently stream levels in/out based on streaming volumes.
...
_property_ `volumetric_lightmap_loading_range`: float

[Read-Write] Range in which volumetric lightmaps will be loaded.
...
_property_ `world_to_meters`: float

[Read-Only] scale of 1uu to 1m in real world measurements, for HMD and other physically tracked devices (e.g. 1uu = 1cm would be 100.0)
...
- `unreal.WorldSettings`
  - `WorldSettings`
    - `WorldSettings.add_asset_user_data_of_class()`
    - `WorldSettings.ai_system_class`
    - `WorldSettings.default_color_scale`
    - `WorldSettings.default_game_mode`
    - `WorldSettings.default_game_type`
    - `WorldSettings.default_physics_volume_class`
    - `WorldSettings.enable_ai_system`
    - `WorldSettings.enable_large_worlds`
    - `WorldSettings.enable_navigation_system`
    - `WorldSettings.enable_world_bounds_checks`
    - `WorldSettings.enable_world_composition`
    - `WorldSettings.enable_world_origin_rebasing`
    - `WorldSettings.get_asset_user_data_of_class()`
    - `WorldSettings.global_gravity_set`
    - `WorldSettings.global_gravity_z`
    - `WorldSettings.has_asset_user_data_of_class()`
    - `WorldSettings.kill_z`
    - `WorldSettings.kill_z_damage_type`
    - `WorldSettings.lightmass_settings`
    - `WorldSettings.navigation_system_config`
    - `WorldSettings.physics_collision_handler_class`
    - `WorldSettings.use_client_side_level_streaming_volumes`
    - `WorldSettings.volumetric_lightmap_loading_range`
    - `WorldSettings.world_to_meters`