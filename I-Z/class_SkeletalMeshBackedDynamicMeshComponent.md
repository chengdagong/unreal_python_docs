# unreal.SkeletalMeshBackedDynamicMeshComponent

```python
class unreal.SkeletalMeshBackedDynamicMeshComponent(outer:Object | None=None, name:Name | str='None')
```


**Bases:** ``DynamicMeshComponent``


Skeletal Mesh Backed Dynamic Mesh Component
...
- **File**: SKMBackedDynaMeshComponent.h
...
- `disable_mesh_uv_hit_results` (bool): [Read-Write] If true, UV hit results will not be enabled for this mesh’s complex collision even if they are enabled for the project settings generally. This can make the complex build faster and take less memory.

- `distance_field_mode` (DynamicMeshComponentDistanceFieldMode): [Read-Write]
deprecated: Dynamic Mesh distance field support has been deprecated
...
- `unreal.SkeletalMeshBackedDynamicMeshComponent`
  - `SkeletalMeshBackedDynamicMeshComponent`