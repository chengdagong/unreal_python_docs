...
```python
class unreal.NiagaraDataInterfaceSceneCapture2D(outer:Object | None=None, name:Name | str='None')
```
...
- `managed_hide_attach_component` (bool): [Read-Write]

- `managed_hiden_actors` (Array[Actor]): [Read-Write]
...
_classmethod_ set_scene_capture2d_managed_show_only_actors( _niagara_system_, _parameter_name_, _show_only_actors_)→None
...
- **show\_only\_actors** ( _Array_ _\_ [_Actor_ _]_)


### Table of Contents

- `unreal.NiagaraDataInterfaceSceneCapture2D`
  - `NiagaraDataInterfaceSceneCapture2D`
    - `NiagaraDataInterfaceSceneCapture2D.set_scene_capture2d_managed_show_only_actors()`