...
```python
class unreal.AnimNode_PoseSearchHistoryCollector_Base(pose_count:int=0, sampling_interval:float=0.0, root_bone_recovery_time:float=0.0, root_bone_translation_recovery_ratio:float=0.0, root_bone_rotation_recovery_ratio:float=0.0, trajectory_history_count:int=0, trajectory_prediction_count:int=0, prediction_sampling_interval:float=0.0)
```
...
_property_ `prediction_sampling_interval`: float

[Read-Write] if bGenerateTrajectory is true, this is the sampling interval between trajectory future (prediction) samples
...
_property_ `root_bone_recovery_time`: float

[Read-Write] time in seconds to recover to the reference skeleton root bone transform by RootBoneTranslationRecoveryRatio and RootBoneRotationRecoveryRatio
from any eventual root bone modification. if zero the behaviour will be disabled
Experimental, this feature might be removed without warning, not for production use
...
_property_ `root_bone_rotation_recovery_ratio`: float

[Read-Write] ratio to recover to the reference skeleton root bone rotation from any eventual root bone modification. zero for no recovery, 1 for full recovery
...
_property_ `root_bone_translation_recovery_ratio`: float

[Read-Write] ratio to recover to the reference skeleton root bone translation from any eventual root bone modification. zero for no recovery, 1 for full recovery
...
_property_ `sampling_interval`: float

[Read-Write] how often in seconds poses are collected (if 0, it will collect every update)
...
_property_ `trajectory_history_count`: int

[Read-Write] if bGenerateTrajectory is true, this is the number of trajectory past (collected) samples
...
_property_ `trajectory_prediction_count`: int

[Read-Write] if bGenerateTrajectory is true, this is the number of trajectory future (prediction) samples
...
- `unreal.AnimNode_PoseSearchHistoryCollector_Base`
  - `AnimNode_PoseSearchHistoryCollector_Base`
    - `AnimNode_PoseSearchHistoryCollector_Base.pose_count`
    - `AnimNode_PoseSearchHistoryCollector_Base.prediction_sampling_interval`
    - `AnimNode_PoseSearchHistoryCollector_Base.root_bone_recovery_time`
    - `AnimNode_PoseSearchHistoryCollector_Base.root_bone_rotation_recovery_ratio`
    - `AnimNode_PoseSearchHistoryCollector_Base.root_bone_translation_recovery_ratio`
    - `AnimNode_PoseSearchHistoryCollector_Base.sampling_interval`
    - `AnimNode_PoseSearchHistoryCollector_Base.trajectory_history_count`
    - `AnimNode_PoseSearchHistoryCollector_Base.trajectory_prediction_count`