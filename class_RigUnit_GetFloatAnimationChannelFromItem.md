# unreal.RigUnit_GetFloatAnimationChannelFromItem

```python
class unreal.RigUnit_GetFloatAnimationChannelFromItem(item:RigElementKey=Ellipsis, initial:bool=False, value:float=0.0)
```


**Bases:** ``RigUnit_GetAnimationChannelFromItemBase``


Get Float Channel is used to retrieve a control’s animation channel value

**C++ Source:**

- **Plugin**: ControlRig

- **Module**: ControlRig

- **File**: RigUnit\_ControlChannelFromItem.h


**Editor Properties:** (see get\_editor\_property/set\_editor\_property)

- `initial` (bool): [Read-Write] If set to true the initial value will be returned

- `item` (RigElementKey): [Read-Write] The item representing the channel

- `value` (float): [Read-Write] The current value of the animation channel



---

_property_ `value`: float

[Read-Only] The current value of the animation channel

**Type:**

( float)

### Table of Contents

- `unreal.RigUnit_GetFloatAnimationChannelFromItem`
  - `RigUnit_GetFloatAnimationChannelFromItem`
    - `RigUnit_GetFloatAnimationChannelFromItem.value`