...
```python
class unreal.StaticMeshComponent(outer:Object | None=None, name:Name | str='None')
```
...
_property_ `cast_distance_field_indirect_shadow`: bool

[Read-Only] Whether to use the mesh distance field representation (when present) for shadowing indirect lighting (from lightmaps or skylight) on Movable components.
This works like capsule shadows on skeletal meshes, except using the mesh distance field so no physics asset is required.
The StaticMesh must have ‘Generate Mesh Distance Field’ enabled, or the project must have ‘Generate Mesh Distance Fields’ enabled for this feature to work.
...
_property_ `disallow_nanite`: bool

[Read-Write] Forces this component to use fallback mesh for rendering if Nanite is enabled on the mesh.
...
_property_ `distance_field_indirect_shadow_min_visibility`: float

[Read-Only] Controls how dark the dynamic indirect shadow can be.
...
_property_ `distance_field_self_shadow_bias`: float

[Read-Only] Useful for reducing self shadowing from distance field methods when using world position offset to animate the mesh’s vertices.
...
_property_ `enable_texture_color_mesh_painting`: bool

[Read-Write] If false, texture color mesh painting is disabled on this component.
...
_property_ `enable_vertex_color_mesh_painting`: bool

[Read-Write] If false, vertex color mesh painting is disabled on this component.
This may be set to false by blueprint functions that override vertex colors in construction script.
...
_property_ `evaluate_world_position_offset`: bool

[Read-Write] Whether to evaluate World Position Offset.
...
_property_ `evaluate_world_position_offset_in_ray_tracing`: bool

[Read-Write] Whether to evaluate World Position Offset for ray tracing.
This is only used when running with r.RayTracing.Geometry.StaticMeshes.WPO=1
...
_property_ `force_nanite_for_masked`: bool

[Read-Write] Forces this component to always use Nanite for masked materials, even if FNaniteSettings::bAllowMaskedMaterials=false
...
_property_ `forced_lod_model`: int

[Read-Only] If 0, auto-select LOD level. if >0, force to (ForcedLodModel-1).
...
**get_initial_evaluate_world_position_offset**() → `bool`
...
get\_local\_bounds( _)->(min=Vector_, _max=Vector_)
...
_property_ `ignore_instance_for_texture_streaming`: bool

[Read-Write] Ignore this instance of this static mesh when calculating streaming information.
This can be useful when doing things like applying character textures to static geometry,
to avoid them using distance-based streaming.
...
_property_ `material_cache_tile_count`: IntPoint

[Read-Write] Number of tiles allocated to the material cache textures, may be modified by tag properties
...
_property_ `min_lod`: int

[Read-Only] Specifies the smallest LOD that will be used for this component.
This is ignored if ForcedLodModel is enabled.
...
_property_ `nanite_pixel_programmable_distance`: float

[Read-Only] Used to forcefully disable pixel programmable rasterization of Nanite when the mesh is further than a given distance from the camera.
...
_property_ `overridden_light_map_res`: int

[Read-Only] Light map resolution to use on this component, used if bOverrideLightMapRes is true and there is a valid StaticMesh.
...
_property_ `overridden_mesh_paint_texture_coordinate_index`: int

[Read-Write] The overriden coordinate index to use when texture color painting on this component.
...
_property_ `overridden_mesh_paint_texture_resolution`: int

[Read-Write] The overriden resolution of texture color mesh paint textures on this component.
...
_property_ `override_distance_field_self_shadow_bias`: bool

[Read-Only] Whether to override the DistanceFieldSelfShadowBias setting of the static mesh asset with the DistanceFieldSelfShadowBias of this component.
...
_property_ `override_light_map_res`: bool

[Read-Only] Whether to override the lightmap resolution defined in the static mesh.
...
_property_ `override_mesh_paint_texture_coordinate_index`: bool

[Read-Write] Whether to override the MeshPaintTextureCoordinateIndex set on the static mesh.
...
_property_ `override_mesh_paint_texture_resolution`: bool
...
_property_ `override_min_lod`: bool

[Read-Only] Whether to override the MinLOD setting of the static mesh asset with the MinLOD of this component.
...
_property_ `override_wireframe_color`: bool

[Read-Only] If true, WireframeColorOverride will be used. If false, color is determined based on mobility and physics simulation settings
...
_property_ `reverse_culling`: bool

[Read-Only] Controls whether the static mesh component’s backface culling should be reversed
...
**set_distance_field_self_shadow_bias**( _new_value_) → `None`
...
**set_evaluate_world_position_offset**( _new_value_) → `None`
...
**set_evaluate_world_position_offset_in_ray_tracing**( _new_value_) → `None`
...
**set_force_disable_nanite**( _force_disable_nanite_) → `None`

Force disabling of Nanite rendering. When true, Will swap to the the fallback mesh instead.

Parameters:

**force\_disable\_nanite** ( _bool_)


---

**set_forced_lod_model**( _new_forced_lod_model_) → `None`

Set Forced Lod Model

Parameters:

**new\_forced\_lod\_model** ( _int32_)


---

**set_reverse_culling**( _reverse_culling_) → `None`

Set forced reverse culling

Parameters:

**reverse\_culling** ( _bool_)


---

**set_static_mesh**( _new_mesh_) → `bool`
...
**set_world_position_offset_disable_distance**( _new_value_) → `None`
...
_property_ `sort_triangles`: bool

[Read-Only] Enable dynamic sort mesh’s triangles to remove ordering issue when rendered with a translucent material
...
_property_ `static_mesh`: StaticMesh

[Read-Only] The static mesh that this component uses to render
...
_property_ `streaming_distance_multiplier`: float

[Read-Write] Allows adjusting the desired resolution of streaming textures that uses UV 0. 1.0 is the default, whereas a higher value increases the streamed-in resolution.
...
**update_initial_evaluate_world_position_offset**() → `None`
...
_property_ `wireframe_color_override`: Color

[Read-Only] Wireframe color to use if bOverrideWireframeColor is true
...
_property_ `world_position_offset_disable_distance`: int

[Read-Only] Distance at which to disable World Position Offset for an entire instance (0 = Never disable WPO).
...
_property_ `world_position_offset_writes_velocity`: bool

[Read-Write] Whether world position offset turns on velocity writes.
If the WPO isn’t static then setting false may give incorrect motion vectors.
But if we know that the WPO is static then setting false may save performance.
...
- `unreal.StaticMeshComponent`
  - `StaticMeshComponent`
    - `StaticMeshComponent.cast_distance_field_indirect_shadow`
    - `StaticMeshComponent.disallow_nanite`
    - `StaticMeshComponent.distance_field_indirect_shadow_min_visibility`
    - `StaticMeshComponent.distance_field_self_shadow_bias`
    - `StaticMeshComponent.enable_texture_color_mesh_painting`
    - `StaticMeshComponent.enable_vertex_color_mesh_painting`
    - `StaticMeshComponent.evaluate_world_position_offset`
    - `StaticMeshComponent.evaluate_world_position_offset_in_ray_tracing`
    - `StaticMeshComponent.force_nanite_for_masked`
    - `StaticMeshComponent.forced_lod_model`
    - `StaticMeshComponent.get_initial_evaluate_world_position_offset()`
    - `StaticMeshComponent.get_local_bounds()`
    - `StaticMeshComponent.ignore_instance_for_texture_streaming`
    - `StaticMeshComponent.material_cache_tile_count`
    - `StaticMeshComponent.min_lod`
    - `StaticMeshComponent.nanite_pixel_programmable_distance`
    - `StaticMeshComponent.overridden_light_map_res`
    - `StaticMeshComponent.overridden_mesh_paint_texture_coordinate_index`
    - `StaticMeshComponent.overridden_mesh_paint_texture_resolution`
    - `StaticMeshComponent.override_distance_field_self_shadow_bias`
    - `StaticMeshComponent.override_light_map_res`
    - `StaticMeshComponent.override_mesh_paint_texture_coordinate_index`
    - `StaticMeshComponent.override_mesh_paint_texture_resolution`
    - `StaticMeshComponent.override_min_lod`
    - `StaticMeshComponent.override_wireframe_color`
    - `StaticMeshComponent.reverse_culling`
    - `StaticMeshComponent.set_distance_field_self_shadow_bias()`
    - `StaticMeshComponent.set_evaluate_world_position_offset()`
    - `StaticMeshComponent.set_evaluate_world_position_offset_in_ray_tracing()`
    - `StaticMeshComponent.set_force_disable_nanite()`
    - `StaticMeshComponent.set_forced_lod_model()`
    - `StaticMeshComponent.set_reverse_culling()`
    - `StaticMeshComponent.set_static_mesh()`
    - `StaticMeshComponent.set_world_position_offset_disable_distance()`
    - `StaticMeshComponent.sort_triangles`
    - `StaticMeshComponent.static_mesh`
    - `StaticMeshComponent.streaming_distance_multiplier`
    - `StaticMeshComponent.update_initial_evaluate_world_position_offset()`
    - `StaticMeshComponent.wireframe_color_override`
    - `StaticMeshComponent.world_position_offset_disable_distance`
    - `StaticMeshComponent.world_position_offset_writes_velocity`