...
```python
class unreal.Texture2D(outer:Object | None=None, name:Name | str='None')
```
...
**blueprint_get_cpu_copy**() → `SharedImageConstRefBlueprint`
...
- `unreal.Texture2D`
  - `Texture2D`
    - `Texture2D.address_x`
    - `Texture2D.address_y`
    - `Texture2D.blueprint_get_cpu_copy()`
    - `Texture2D.blueprint_get_size_x()`
    - `Texture2D.blueprint_get_size_y()`