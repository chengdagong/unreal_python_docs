...
- `use_default_as_simulated` (bool): [Read-Write] Treat parts in the Physics Asset with PhysicsType Default as Simulated for RBAN.
...
_property_ `use_local_lod_threshold_only`: bool

[Read-Write] Enable if you want to ignore the p.RigidBodyLODThreshold CVAR and force the node to solely use the LOD threshold.
...
- `unreal.AnimNode_RigidBody`
  - `AnimNode_RigidBody`
    - `AnimNode_RigidBody.use_local_lod_threshold_only`