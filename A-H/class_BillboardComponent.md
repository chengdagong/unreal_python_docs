...
```python
class unreal.BillboardComponent(outer:Object | None=None, name:Name | str='None')
```
...
_property_ `is_screen_size_scaled`: bool
...
_property_ `opacity_mask_ref_val`: float

[Read-Write] The billboard is not rendered where texture opacity < OpacityMaskRefVal
...
**set_opacity_mask_ref_val**( _ref_val_) → `None`
...
**set_sprite**( _new_sprite_) → `None`

Change the sprite texture used by this component

Parameters:

**new\_sprite** ( _Texture2D_)


---

**set_sprite_and_uv**( _new_sprite_, _new_u_, _new_ul_, _new_v_, _new_vl_) → `None`
...
**set_uv**( _new_u_, _new_ul_, _new_v_, _new_vl_) → `None`
...
_property_ `use_in_editor_scaling`: bool

[Read-Write] Whether to use in-editor arrow scaling (i.e. to be affected by the global arrow scale)
...
- `unreal.BillboardComponent`
  - `BillboardComponent`
    - `BillboardComponent.is_screen_size_scaled`
    - `BillboardComponent.opacity_mask_ref_val`
    - `BillboardComponent.screen_size`
    - `BillboardComponent.set_opacity_mask_ref_val()`
    - `BillboardComponent.set_sprite()`
    - `BillboardComponent.set_sprite_and_uv()`
    - `BillboardComponent.set_uv()`
    - `BillboardComponent.sprite`
    - `BillboardComponent.u`
    - `BillboardComponent.ul`
    - `BillboardComponent.use_in_editor_scaling`
    - `BillboardComponent.v`
    - `BillboardComponent.vl`