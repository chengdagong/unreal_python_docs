...
```python
class unreal.GameState(outer:Object | None=None, name:Name | str='None')
```
...
- `server_world_time_seconds_update_frequency` (float): [Read-Write] How often the server updates the replicated TimeSeconds from the world. Values mean “update every X seconds”, so 0.5 means update every half second. Set to zero to disable periodic updates.
...
_property_ `elapsed_time`: int

[Read-Only] Elapsed game time since match has started.
...
_property_ `match_state`: Name

[Read-Only] What match state we are currently in
...
_property_ `previous_match_state`: Name

[Read-Only] Previous map state, used to handle if multiple transitions happen per frame
...
- `unreal.GameState`
  - `GameState`
    ...
    - `GameState.previous_match_state`