...
```python
class unreal.CineCameraComponent(outer:Object | None=None, name:Name | str='None')
```


**Bases:** ``CameraComponent``
...
_property_ `crop_settings`: PlateCropSettings

[Read-Write] Controls the crop settings.
...
_property_ `current_aperture`: float

[Read-Write] Current aperture, in terms of f-stop (e.g. 2.8 for f/2.8)
...
_property_ `current_focal_length`: float

[Read-Write] Current focal length of the camera (i.e. controls FoV, zoom)
...
_property_ `current_focus_distance`: float

[Read-Only] Read-only. Control this value via FocusSettings.
...
_property_ `exposure_method`: CameraExposureMethod

[Read-Write] Exposure method, for determining whether exposure-related post-process settings should be automatically set from camera parameters

**Type:**

( CameraExposureMethod)


---

_property_ `filmback`: CameraFilmbackSettings

[Read-Write] Controls the filmback of the camera.
...
_property_ `focus_settings`: CameraFocusSettings

[Read-Write] Controls the camera’s focus.
...
**get_default_filmback_preset_name**() → `str`
...
_classmethod_ get_filmback_presets_copy()→Array\ [NamedFilmbackPreset]
...
**get_horizontal_projection_offset**() → `float`
...
_classmethod_ get_lens_presets_copy()→Array\ [NamedLensPreset]
...
**get_vertical_projection_offset**() → `float`
...
_property_ `lens_settings`: CameraLensSettings

[Read-Write] Controls the camera’s lens.
...
**set_crop_preset_by_name**( _preset_name_) → `None`
...
**set_filmback_preset_by_name**( _preset_name_) → `None`
...
**set_lens_preset_by_name**( _preset_name_) → `None`
...
- `unreal.CineCameraComponent`
  - `CineCameraComponent`
    - `CineCameraComponent.crop_settings`
    - `CineCameraComponent.current_aperture`
    - `CineCameraComponent.current_focal_length`
    - `CineCameraComponent.current_focus_distance`
    - `CineCameraComponent.custom_near_clipping_plane`
    - `CineCameraComponent.exposure_method`
    - `CineCameraComponent.filmback`
    - `CineCameraComponent.focus_settings`
    - `CineCameraComponent.get_crop_preset_name()`
    - `CineCameraComponent.get_default_filmback_preset_name()`
    - `CineCameraComponent.get_filmback_preset_name()`
    - `CineCameraComponent.get_filmback_presets_copy()`
    - `CineCameraComponent.get_horizontal_field_of_view()`
    - `CineCameraComponent.get_horizontal_projection_offset()`
    - `CineCameraComponent.get_lens_preset_name()`
    - `CineCameraComponent.get_lens_presets_copy()`
    - `CineCameraComponent.get_vertical_field_of_view()`
    - `CineCameraComponent.get_vertical_projection_offset()`
    - `CineCameraComponent.lens_settings`
    - `CineCameraComponent.override_custom_near_clipping_plane`
    - `CineCameraComponent.set_crop_preset_by_name()`
    - `CineCameraComponent.set_filmback_preset_by_name()`
    - `CineCameraComponent.set_lens_preset_by_name()`