...
```python
class unreal.DisplayClusterRootActor(outer:Object | None=None, name:Name | str='None')
```
...
Must have CPF\_Edit(such as VisibleDefaultsOnly) on property for Live Link.
...
- `global_inner_frustum_upscaler_settings_ref` (DisplayClusterEditorPropertyReference): [Read-Write]
...
- `outer_viewport_upscaler_settings_ref` (DisplayClusterEditorPropertyReference): [Read-Write]

- `per_outer_hide_list_ref` (DisplayClusterEditorPropertyReference): [Read-Write] Hide actors from a specific viewport.
...
- `preview_enable_holdout_composite` (bool): [Read-Write] Render a preview with postprocessing in the scene using the HoldoutComposite plugin.
...
- `preview_enable_ocio` (bool): [Read-Write] Enable OCIO on preview.
...
- `preview_enable_tsr` (bool): [Read-Write] Enable TemporalAA/TSR in preview.
...
- `viewport_upscaler_settings_ref` (DisplayClusterEditorPropertyReference): [Read-Write] Adjust upscaler settings for an individual viewport.
...
_property_ `current_config_data`: DisplayClusterConfigurationData
...
_property_ `default_view_point`: DisplayClusterCameraComponent

[Read-Only] Default camera component. It’s an outer camera in VP/ICVFX terminology. Always exists on a DCRA instance.
...
_property_ `enable_preview_techvis`: bool

[Read-Write] Configure the root actor for Techvis rendering with preview components.
...
_property_ `freeze_preview_render`: bool

[Read-Write] Freeze preview render. This will impact editor performance.
...
**get_common_view_point**() → `SceneComponent`
...
**get_distance_to_stage_geometry**( _world_position_, _world_direction_) → `floatorNone`
...
- **world\_position** ( _Vector_) – The world position to measure the distance from

- **world\_direction** ( _Vector_) – The direction to find the distance to the geometry along


Returns:

True if an intersection point from WorldPosition along WorldDirection was found, false if not

out\_distance (float): The distance to the stage geometry from the specified point
...
**get_flush_position_and_normal**( _world_position_) → `(out_position=Vector,out_normal=Vector)orNone`

Get Flush Position and Normal

Parameters:

**world\_position** ( _Vector_)

Returns:

out\_position (Vector):
...
**make_stage_actor_flush_to_wall**( _stage_actor_, _desired_offset_from_flush=0.000000_) → `bool`

Make Stage Actor Flush to Wall

Parameters:

- **stage\_actor** ( _DisplayClusterStageActor_)

- **desired\_offset\_from\_flush** ( _double_)
...
_property_ `preview_enable`: bool

[Read-Write] Render the scene and display it as a preview on the nDisplay root actor in the editor. This will impact editor performance.
...
_property_ `preview_enable_holdout_composite`: bool

[Read-Write] Render a preview with postprocessing in the scene using the HoldoutComposite plugin.
...
_property_ `preview_enable_ocio`: bool

[Read-Write] Enable OCIO on preview.
...
_property_ `preview_enable_overlay_material`: bool

[Read-Write] Show overlay material on the preview mesh when preview rendering is enabled (UMeshComponent::OverlayMaterial).
...
_property_ `preview_enable_post_process`: bool

[Read-Write] Enable PostProcess for preview.
...
_property_ `preview_enable_tsr`: bool

[Read-Write] Enable TemporalAA/TSR in preview.
Increases memory consumption. This will impact editor performance.
...
_property_ `preview_icvfx_frustums`: bool

[Read-Write] Render ICVFX Frustums
...
_property_ `preview_icvfx_frustums_far_distance`: float
...
_property_ `preview_in_game_enable`: bool

[Read-Write] Render this DCRA in game for Standalone/Package builds.
...
_property_ `preview_max_texture_dimension`: int

[Read-Write] The maximum dimension of any internal texture for preview. Use less memory for large preview viewports
...
_property_ `preview_node_id`: str

[Read-Write] Selectively preview a specific viewport or show all/none.
...
_property_ `preview_render_target_ratio_mult`: float

[Read-Write] Adjust resolution scaling for the editor preview.
...
_property_ `render_mode`: DisplayClusterConfigurationRenderMode

[Read-Write] Render Mode

**Type:**

( DisplayClusterConfigurationRenderMode)


---

**set_freeze_outer_viewports**( _enable_) → `bool`
...
**set_replace_texture_flag_for_all_viewports**( _replace_) → `bool`
...
_property_ `tick_per_frame`: int

[Read-Write] Tick Per Frame
...
**update_procedural_mesh_component_data**( _procedural_mesh_component=None_) → `None`

Update the geometry of the procedural mesh component(s) referenced inside nDisplay

Parameters:

**procedural\_mesh\_component** ( _ProceduralMeshComponent_) – (optional) Mark the specified procedural mesh component, not all


---

_property_ `viewports_per_frame`: int

[Read-Write] Max amount of Viewports Per Frame
...
- `unreal.DisplayClusterRootActor`
  - `DisplayClusterRootActor`
    - `DisplayClusterRootActor.current_config_data`
    - `DisplayClusterRootActor.default_view_point`
    - `DisplayClusterRootActor.enable_preview_techvis`
    - `DisplayClusterRootActor.freeze_preview_render`
    - `DisplayClusterRootActor.get_common_view_point()`
    - `DisplayClusterRootActor.get_distance_to_stage_geometry()`
    - `DisplayClusterRootActor.get_flush_position_and_normal()`
    - `DisplayClusterRootActor.make_stage_actor_flush_to_wall()`
    - `DisplayClusterRootActor.preview_enable`
    - `DisplayClusterRootActor.preview_enable_holdout_composite`
    - `DisplayClusterRootActor.preview_enable_ocio`
    - `DisplayClusterRootActor.preview_enable_overlay_material`
    - `DisplayClusterRootActor.preview_enable_post_process`
    - `DisplayClusterRootActor.preview_enable_tsr`
    - `DisplayClusterRootActor.preview_icvfx_frustums`
    - `DisplayClusterRootActor.preview_icvfx_frustums_far_distance`
    - `DisplayClusterRootActor.preview_in_game_enable`
    - `DisplayClusterRootActor.preview_max_texture_dimension`
    - `DisplayClusterRootActor.preview_node_id`
    - `DisplayClusterRootActor.preview_render_target_ratio_mult`
    - `DisplayClusterRootActor.render_mode`
    - `DisplayClusterRootActor.set_freeze_outer_viewports()`
    - `DisplayClusterRootActor.set_replace_texture_flag_for_all_viewports()`
    - `DisplayClusterRootActor.tick_per_frame`
    - `DisplayClusterRootActor.update_procedural_mesh_component_data()`
    - `DisplayClusterRootActor.viewports_per_frame`