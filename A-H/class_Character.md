...
```python
class unreal.Character(outer:Object | None=None, name:Name | str='None')
```
...
This is incremented in CheckJumpInput, used in CanJump\_Implementation, and reset in OnMovementModeChanged.
...
**cache_initial_mesh_offset**( _mesh_relative_location_, _mesh_relative_rotation_) → `None`
...
- **mesh\_relative\_location** ( _Vector_)

- **mesh\_relative\_rotation** ( _Rotator_)



---

**can_crouch**() → `boolReturns:`
...
_property_ `capsule_component`: CapsuleComponent

[Read-Only] The CapsuleComponent being used for movement collision (by CharacterMovement). Always treated as being vertically aligned in simple collision check functions.
...
_property_ `character_movement`: CharacterMovementComponent

[Read-Only] Movement component used for movement logic in various movement modes (walking, falling, etc), containing relevant settings and functions to control movement.
...
**crouch**( _client_simulation=False_) → `None`
...
**client\_simulation** ( _bool_)


---

_property_ `crouched_eye_height`: float

[Read-Write] Default crouched eye height
...
**get_anim_root_motion_translation_scale**() → `float`
...
**get_base_rotation_offset**() → `Rotator`
...
**get_base_translation_offset**() → `Vector`
...
_property_ `is_crouched`: bool

[Read-Only] Set by character movement to specify that this Character is currently crouched.
...
deprecated: ‘is\_jumping’ was renamed to ‘is\_jump\_providing\_force’.


---

**is_playing_networked_root_motion_montage**() → `bool`
...
_property_ `jump_current_count`: int

[Read-Only] Tracks the current number of jumps performed.
This is incremented in CheckJumpInput, used in CanJump\_Implementation, and reset in OnMovementModeChanged.
When providing overrides for these methods, it’s recommended to either manually
increment / reset this value, or call the Super:: method.
...
_property_ `jump_current_count_pre_jump`: int

[Read-Only] Represents the current number of jumps performed before CheckJumpInput modifies JumpCurrentCount.
This is set in CheckJumpInput and is used in SetMoveFor and PrepMoveFor instead of JumpCurrentCount
since CheckJumpInput can modify JumpCurrentCount.
When providing overrides for these methods, it’s recommended to either manually
set this value, or call the Super:: method.
...
_property_ `jump_force_time_remaining`: float

[Read-Only] Amount of jump force time remaining, if JumpMaxHoldTime > 0.
...
_property_ `jump_key_hold_time`: float

[Read-Only] Jump key Held Time.
This is the time that the player has held the jump key, in seconds.
...
_property_ `jump_max_count`: int

[Read-Write] The max number of jumps the character can perform.
Note that if JumpMaxHoldTime is non zero and StopJumping is not called, the player
may be able to perform and unlimited number of jumps. Therefore it is usually
best to call StopJumping() when jump input has ceased (such as a button up event).
...
_property_ `jump_max_hold_time`: float

[Read-Write] The max time the jump key can be held.
Note that if StopJumping() is not called before the max jump hold time is reached,
then the character will carry on receiving vertical velocity. Therefore it is usually
best to call StopJumping() when jump input has ceased (such as a button up event).
...
_property_ `landed_delegate`: LandedSignature

[Read-Write] Called upon landing when falling, to perform actions based on the Hit result.
Note that movement mode is still “Falling” during this event. Current Velocity value is the velocity at the time of landing.
Consider OnMovementModeChanged() as well, as that can be used once the movement mode changes to the new mode (most likely Walking).
see: OnMovementModeChanged()
...
**launch**( _launch_velocity:Vector_, _xy_override:bool_, _z_override:bool_) → `None`

deprecated: ‘launch’ was renamed to ‘launch\_character’.


---

**launch_character**( _launch_velocity_, _xy_override_, _z_override_) → `None`
...
- **launch\_velocity** ( _Vector_) – is the velocity to impart to the Character

- **xy\_override** ( _bool_) – if true replace the XY part of the Character’s velocity instead of adding to it.

- **z\_override** ( _bool_) – if true replace the Z component of the Character’s velocity instead of adding to it.



---

_property_ `mesh`: SkeletalMeshComponent

[Read-Only] The main skeletal mesh associated with this Character (optional sub-object).
...
_property_ `movement_mode_changed_delegate`: MovementModeChangedSignature

[Read-Write] Multicast delegate for MovementMode changing.
...
_property_ `on_character_movement_updated`: CharacterMovementUpdatedSignature

[Read-Write] Event triggered at the end of a CharacterMovementComponent movement update.
This is the preferred event to use rather than the Tick event when performing custom updates to CharacterMovement properties based on the current state.
This is mainly due to the nature of network updates, where client corrections in position from the server can cause multiple iterations of a movement update,
which allows this event to update as well, while a Tick event would not.
...
**on_end_crouch**( _half_height_adjust_, _scaled_half_height_adjust_) → `None`

Event when Character stops crouching.

Parameters:

- **half\_height\_adjust** ( _float_) – difference between default collision half-height, and actual crouched capsule half-height.

- **scaled\_half\_height\_adjust** ( _float_) – difference after component scale is taken in to account.
...
**hit** ( _HitResult_) – Result describing the landing that resulted in a valid landing spot.


---

**on_launched**( _launch_velocity_, _xy_override_, _z_override_) → `None`

Let blueprint know that we were launched

Parameters:

- **launch\_velocity** ( _Vector_)
...
**on_movement_mode_changed**( _prev_movement_mode_, _new_movement_mode_, _prev_custom_mode_, _new_custom_mode_) → `None`

Called from CharacterMovementComponent to notify the character that the movement mode has changed.

Parameters:

- **prev\_movement\_mode** ( _MovementMode_) – Movement mode before the change

- **new\_movement\_mode** ( _MovementMode_) – New movement mode

- **prev\_custom\_mode** ( _uint8_) – Custom mode before the change (applicable if PrevMovementMode is Custom)

- **new\_custom\_mode** ( _uint8_) – New custom mode (applicable if NewMovementMode is Custom)



---

_property_ `on_reached_jump_apex`: CharacterReachedApexSignature

[Read-Write] Broadcast when Character’s jump reaches its apex. Needs CharacterMovement->bNotifyApex = true
...
**on_start_crouch**( _half_height_adjust_, _scaled_half_height_adjust_) → `None`
...
**on_walking_off_ledge**( _previous_floor_impact_normal_, _previous_floor_contact_normal_, _previous_location_, _time_delta_) → `None`
...
- **previous\_floor\_impact\_normal** ( _Vector_) – Normal of the previous walkable floor.

- **previous\_floor\_contact\_normal** ( _Vector_) – Normal of the contact with the previous walkable floor.

- **previous\_location** ( _Vector_) – Previous character location before movement off the ledge.
...
**play_anim_montage**( _anim_montage_, _play_rate=1.000000_, _start_section_name='None'_) → `float`
...
- **anim\_montage** ( _AnimMontage_)

- **play\_rate** ( _float_)

- **start\_section\_name** ( _Name_)
...
_property_ `pressed_jump`: bool

[Read-Only] When true, player wants to jump
...
_property_ `proxy_jump_force_started_time`: float

[Read-Only] Track last time a jump force started for a proxy.
...
**stop_anim_montage**( _anim_montage=None_) → `None`
...
**un_crouch**( _client_simulation=False_) → `None`
...
**update_custom_movement**( _delta_time_) → `None`
...
_property_ `was_jumping`: bool

[Read-Only] Tracks whether or not the character was already jumping last frame.
...
- `unreal.Character`
  - `Character`
    - `Character.cache_initial_mesh_offset()`
    - `Character.can_crouch()`
    - `Character.can_jump()`
    - `Character.can_jump_internal()`
    - `Character.capsule_component`
    - `Character.character_movement`
    - `Character.crouch()`
    - `Character.crouched_eye_height`
    - `Character.get_anim_root_motion_translation_scale()`
    - `Character.get_base_rotation_offset()`
    - `Character.get_base_translation_offset()`
    - `Character.get_current_montage()`
    - `Character.has_any_root_motion()`
    - `Character.is_crouched`
    - `Character.is_jump_providing_force()`
    - `Character.is_jumping()`
    - `Character.is_playing_networked_root_motion_montage()`
    - `Character.is_playing_root_motion()`
    - `Character.jump()`
    - `Character.jump_current_count`
    - `Character.jump_current_count_pre_jump`
    - `Character.jump_force_time_remaining`
    - `Character.jump_key_hold_time`
    - `Character.jump_max_count`
    - `Character.jump_max_hold_time`
    - `Character.landed_delegate`
    - `Character.launch()`
    - `Character.launch_character()`
    - `Character.mesh`
    - `Character.movement_mode_changed_delegate`
    - `Character.on_character_movement_updated`
    - `Character.on_end_crouch()`
    - `Character.on_jumped()`
    - `Character.on_landed()`
    - `Character.on_launched()`
    - `Character.on_movement_mode_changed()`
    - `Character.on_reached_jump_apex`
    - `Character.on_start_crouch()`
    - `Character.on_walking_off_ledge()`
    - `Character.play_anim_montage()`
    - `Character.pressed_jump`
    - `Character.proxy_jump_force_started_time`
    - `Character.stop_anim_montage()`
    - `Character.stop_jumping()`
    - `Character.un_crouch()`
    - `Character.update_custom_movement()`
    - `Character.was_jumping`