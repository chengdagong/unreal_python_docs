...
```python
class unreal.AnimNode_PoseSearchHistoryCollector(pose_count:int=0, sampling_interval:float=0.0, root_bone_recovery_time:float=0.0, root_bone_translation_recovery_ratio:float=0.0, root_bone_rotation_recovery_ratio:float=0.0, trajectory_history_count:int=0, trajectory_prediction_count:int=0, prediction_sampling_interval:float=0.0, source:PoseLink=\[\])
```
...
- `unreal.AnimNode_PoseSearchHistoryCollector`
  - `AnimNode_PoseSearchHistoryCollector`
    - `AnimNode_PoseSearchHistoryCollector.source`