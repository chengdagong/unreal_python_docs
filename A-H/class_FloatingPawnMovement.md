# unreal.FloatingPawnMovement

```python
class unreal.FloatingPawnMovement(outer:Object | None=None, name:Name | str='None')
```
...
_property_ `accel_rate`: float

‘accel\_rate’ was renamed to ‘acceleration’.
...
_property_ `acceleration`: float

[Read-Write] Acceleration applied by input (rate of change of velocity)
...
_property_ `decel_rate`: float

‘decel\_rate’ was renamed to ‘deceleration’.
...
_property_ `deceleration`: float

[Read-Write] Deceleration applied when there is no input (rate of change of velocity)
...
_property_ `max_speed`: float

[Read-Write] Maximum velocity magnitude allowed for the controlled Pawn.
...
_property_ `turning_boost`: float

[Read-Write] Setting affecting extra force applied when changing direction, making turns have less drift and become more responsive.
Velocity magnitude is not allowed to increase, that only happens due to normal acceleration. It may decrease with large direction changes.
Larger values apply extra force to reach the target direction more quickly, while a zero value disables any extra turn force.
...
- `unreal.FloatingPawnMovement`
  - `FloatingPawnMovement`
    - `FloatingPawnMovement.accel_rate`
    - `FloatingPawnMovement.acceleration`
    - `FloatingPawnMovement.decel_rate`
    - `FloatingPawnMovement.deceleration`
    - `FloatingPawnMovement.max_speed`
    - `FloatingPawnMovement.turning_boost`